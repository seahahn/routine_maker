...
class ToggleEditTextView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : FrameLayout(context, attrs, defStyleAttr) {
    ...
    fun setEditing(editing: Boolean, animate: Boolean, b: Boolean) {
        if (editing != this.editing) {
            this.editing = editing
            d(TAG, "editing : $editing")
            if (!this.editing) {
                textView.text = editText.text

                // 사용자 정보 바꾸기
                // SP에 바뀐 정보 넣기
            } // Set editText text to textView
...
//        d(TAG, "text : $text")
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        ...
        if (state is Bundle) {
            ...
            setEditing(state.getBoolean(EDIT_KEY), false, true)
