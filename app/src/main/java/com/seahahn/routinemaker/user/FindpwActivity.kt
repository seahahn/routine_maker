...
class FindpwActivity : User() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val textView2 = findViewById<TextView>(R.id.tvVerifynum)
        val verifynumInput = findViewById<TextInputEditText>(R.id.verifynumInput)
        val time = findViewById<TextView>(R.id.time)
        val verifynum = verifynumInput.text
        ...
        sendbtn.setOnClickListener {
            ...
            if(resetPw(service, email.toString())) {
                ...
                verifynumInput.visibility = View.VISIBLE
                ...
                countDownTimer = object : CountDownTimer(MILLISINFUTURE.toLong(), COUNT_DOWN_INTERVAL.toLong()) {
                    ...
                    override fun onTick(millisUntilFinished: Long) { //(300초에서 1초 마다 계속 줄어듬)
                        ...
                        if (emailAuthCount - emailAuthCount / 60 * 60 >= 10) { //초가 10보다 크면 그냥 출력
                            time.text = (emailAuthCount / 60).toString() + " : " + (emailAuthCount - emailAuthCount / 60 * 60)
                        } else { //초가 10보다 작으면 앞에 '0' 붙여서 같이 출력. ex) 02,03,04...
                            time.text = (emailAuthCount / 60).toString() + " : 0" + (emailAuthCount - emailAuthCount / 60 * 60)
                        }
    ...
    // 인증 요청
    private fun resetPwCheck(service: RetrofitService, email: String, hash: String): Boolean {
        service.resetPwCheck(email, hash).enqueue(object : Callback<JsonObject> {
            override fun onFailure(call: Call<JsonObject>, t: Throwable) {
